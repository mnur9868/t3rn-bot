...
const { transactionData, delay } = require('./chains/bssp/helper');
const { getAmount } = require('./chains/bssp/api');

const TOKEN_FILE_PATH = path.join(__dirname, 'BSSP_TX_HASH.txt');

const PRIVATE_KEYS = JSON.parse(fs.readFileSync('privateKeys.json', 'utf-8'));
const RPC_URL = T3RN_ABI.at(-1).RPC_BSSP;

const provider = new JsonRpcProvider(RPC_URL);
const CONTRACT_ADDRESS = T3RN_ABI.at(-1).CA_BSSP;
(async () => {
  displayHeader();
          ...
          try {
            ...
            console.log(
              `✅ [ ${moment().format(
                ...
                options === '1' ? 'Arbitrum' : options === '2' ? 'Blast' : 'Optimism'
              } Sepolia!`.green
            );
            console.log(
              `🔗 [ ${moment().format(
            ...
            console.log(
              '✅ Transaction hash url has been saved to BSSP_TX_HASH.txt.'
                .green
            );
            console.log('');
